...
//	gsmm = gsm;
	...
	for (int i = 0; i < 20; i++) 
	{
		theTeapotss.push_back(new Teapot());
	}
}
...
void GameScreenLevel3::DrawCourt()
{
...
void GameScreenLevel3::OutputLine(float x, float y, string text)
{
...
void GameScreenLevel3::Render()
{
	...
	glRotatef(-45, 0.0, 1.0, 0.0);
		...
		glPushMatrix();
			...
			glRotatef(90, 1.0, 0.0, 0.0);
			for (int i = 0; i < theTeapotss.size(); i++)
			{
				theTeapotss[i]->Render();
			}
		glPopMatrix();
...
void GameScreenLevel3::Update(float deltaTime, SDL_Event e)
{
	...
	for (int i = 0; i < theTeapotss.size(); i++) 
	{
		theTeapotss[i]->Update(deltaTime);
	}

	// check for collisions
	for (int i = 0; i < theTeapotss.size(); i++) 
	{
		Collision::SphereSphereCollision(playerboundingSphere, theTeapotss[i]->GetBoundingSphere());
	}
